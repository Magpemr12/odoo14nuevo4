...
class AccountPayment(models.Model):
    ...
    l10n_mx_edi_tax_withholding_id = fields.Many2one(
        'account.tax', 'Tax Withholding', tracking=True, domain=[('type_tax_use', '=', 'purchase')],
    ...
    l10n_mx_edi_tax_withholding_type = fields.Selection([
        ('provisional', 'Pago provisional'), ('definitivo', 'Pago definitivo')], 'Tax Withholding Type',
        tracking=True, help='Indicate the tax withholding type to set in the CFDI.')
    ...
    l10n_mx_edi_tax_withholding_rate = fields.Float(
        'Rate Tax Withholding', digits=(12, 4), compute='_compute_tax_withholding_rate',
        inverse='_inverse_tax_withholding_rate', store=True,
    ...
    def _compute_tax_withholding_rate(self):
        for record in self:
            record.l10n_mx_edi_tax_withholding_rate = record.currency_id._convert(
                1, record.company_id.currency_id, record.company_id, record.date
            ) if record.company_id.currency_id and record.company_id.currency_id != record.currency_id else 1
    ...
    def action_post(self):
        ...
        for payment in self.filtered(lambda p: p.country_code == 'MX' and p.payment_type == 'outbound' and
                                     p.l10n_mx_edi_is_tax_withholding).mapped('move_id'):
