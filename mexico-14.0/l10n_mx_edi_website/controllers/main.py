...
class WebsiteCFDI(Controller):
        ...
        try:
            with request.env.cr.savepoint():
                values = values if values.get('error') else request.env[
                    'pos.order'].sudo().update_partner(ticket_number, vat, email)
        except Exception as msg_error:
            _logger.error(msg_error)
    ...
    def _download_attached_file(self, ticket, ftype='pdf'):
        ...
        with api.Environment.manage():
            ...
            attachment = inv._get_l10n_mx_edi_signed_edi_document().attachment_id if inv and ftype == 'xml' else env[
                'ir.attachment'].search(
                    [('res_model', '=', 'account.move'), ('res_id', '=', inv.id),
                     ('name', '=', ('%s.pdf' % inv._get_report_base_filename()).replace('/', '_'))],
                    limit=1) if inv else False
            if (inv and inv.edi_state == 'sent' and not attachment and ftype == 'pdf'):
                ...
                attachment = inv and env['ir.attachment'].search(
                    [('res_model', '=', 'account.move'),
                     ('res_id', '=', inv.id),
                     ('name', '=', ('%s.pdf' % inv._get_report_base_filename()).replace('/', '_'))], limit=1)
